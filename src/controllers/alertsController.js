...
exports.addAlert = async (req, res) => {
  try {
    ...
    await newAlert.save();
    try {
      req.io.to(userId).emit("new-alert", newAlert);
    } catch (err) {
      console.error("Failed to emit alert:", err);
    }
    res
      .status(201)
      .json({ message: "Alert added successfully", data: newAlert });
  } catch (error) {
...
exports.clearAllAlerts = async (req, res) => {
  try {
    ...
    req.io.to(userId).emit("alerts-cleared");
    res
      .status(200)
      .json({ message: "All alerts cleared successfully for user" });
  } catch (error) {
    res.status(500).json({ message: "Server error", error: error.message });
  }
};
